...
1. Download `rpi_chirpstack_lora_868_example.zip` from here: https://ftp.olimex.com/LoRa/rpi_chirpstack_lora_868_example.zip and extract the image file.
...
- **SSH:** `ssh olimex@olimexpi.local` (password: `olimex`)
- **ChirpStack Web UI:** open [http://olimexpi.local:8080/](http://olimexpi.local:8080/) in a browser (login: `admin` / `admin`)

The image comes with the gateway, device profile, and a test device already configured — no additional ChirpStack setup is needed.
...
**Resetting the session:** if the frame counter gets out of sync with ChirpStack (e.g. after re-flashing the device in ChirpStack), type `R` + ENTER in the Serial Monitor to clear the saved session, then restart the board.

## Verifying in ChirpStack

Open [http://olimexpi.local:8080/](http://olimexpi.local:8080/) and navigate to the pre-configured device. Under the **Events** tab you should see the incoming uplinks with the `TEST` payload and the corresponding downlink ACKs.
